    ...
    def wrapper(event: Any, context: LambdaContext) -> MutableMapping[str, Any]:
        ...
        with (
            ...
            user_future = executor.submit(
                wrap_durable_function(plugin_executor, func, durable_context),
                input_event,
            )
...
def wrap_durable_function(
    plugin_executor: PluginExecutor,
    user_durable_func: Callable[[Any, DurableContext], Any] | None,
    durable_context: DurableContext,
):
    plugin_executor.on_invocation_start(
        durable_context.state.durable_execution_arn,
        durable_context.state.is_replaying(),
        durable_context.state.get_execution_operation().start_timestamp,
        durable_context.lambda_context,
    )

    def wrapped(input_event: Any) -> Any:
        return user_durable_func(input_event, durable_context)
