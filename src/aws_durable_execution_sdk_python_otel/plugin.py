...
from opentelemetry.trace import (
    Tracer, StatusCode, SpanContext, TraceFlags, NonRecordingSpan, Span, SpanKind
)
...
from aws_durable_execution_sdk_python.plugin import (
    ...
    InvocationStartInfo,
    OperationEndInfo,
    OperationStartInfo, AttemptEndInfo, AttemptStartInfo,
)
...
from aws_durable_execution_sdk_python_otel.deterministic_id_generator import (
    DeterministicIdGenerator, operation_id_to_span_id,
)
...
def _should_sample_execution(execution_arn: str, sampling_rate: float) -> bool:
    """Determine whether to sample this execution based on a deterministic hash.

    Uses FNV-1a hash to produce a consistent sampling decision for the same
    execution ARN across all invocations.
    """
    ...
    if sampling_rate <= 0.0:
        return False
    hash_val = 0x811C9DC5
    for ch in execution_arn:
        hash_val ^= ord(ch)
        hash_val = (hash_val * 0x01000193) & 0xFFFFFFFF
    return (hash_val / 0xFFFFFFFF) < sampling_rate
...
class DurableExecutionOtelPlugin(DurableExecutionPlugin):
    ...
    def __init__(
        ...
        self._tracer: Tracer = self._provider.get_tracer(self.INSTRUMENT_NAME)
    ...
    def _resolve_parent_span(self, parent_id: str | None = None) -> Span:
        ...
        logger.info("created a placeholder span for missing parent: %s", placeholder_span)
    ...
    def _start_span(
        ...
        logger.info("starting a span: operation_id=%s, name=%s, parent_span=%s",
                    operation_id, name, parent_span)
        self._id_generator.set_next_span_id(operation_id)
        if parent_span is not None:
            parent_context = trace.set_span_in_context(parent_span, self._extracted_context)
        else:
        ...
        logger.info("started a span: %s at %s", span, _to_otel_timestamp(start_time))
    ...
    def _end_span(self,
        ...
        logger.info(f"ending a span for operation: %s", operation_id)
        ...
        if span:
            ...
            logger.info(f"ended otel span: %s at %s", span, _to_otel_timestamp(end_timestamp))
    ...
    def on_invocation_start(self, info: InvocationStartInfo) -> None:
        ...
        logger.info(f"Invocation started: {info}")
    ...
    def on_operation_end(self, info: OperationEndInfo) -> None:
        ...
        attributes: dict[str, str] = {
            "durable.execution.arn": self._execution_arn,
            "durable.operation.id": info.operation_id,
            "durable.operation.type": info.operation_type.value,
        }
        if info.name:
    ...
    def on_operation_attempt_end(self, info: AttemptEndInfo) -> None:
        ...
        logger.info(f"Attempt ended: {info}")
        ...
        if info.name:
            attributes["durable.operation.name"] = f"{info.name}-attempt-{info.attempt}"
        ...
        # Map the succeeded field to an outcome string
        outcome = "succeeded" if info.succeeded else "failed"
        span.set_attribute("durable.attempt.outcome", outcome)
