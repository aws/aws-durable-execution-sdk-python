...
def _write_metadata(tmp_path: Path, sdk_version: str) -> Path:
    metadata = tmp_path / "lambda-layer-publish.toml"
    metadata.write_text(f'[layer]\nsdk-version = "{sdk_version}"\n')
    return metadata


def test_otel_only_release_uses_pinned_published_sdk(tmp_path: Path) -> None:
    metadata = _write_metadata(tmp_path, "1.8.0")
...
def test_combined_release_uses_source_sdk(tmp_path: Path) -> None:
    metadata = _write_metadata(tmp_path, "1.9.0")
