...
def test_success_on_first_attempt_returns_result_without_retry_strategy():
    ...
    # Patch to track strategy invocations
...
def test_child_context_config_is_forwarded():
    ...
    config = WithRetryConfig(
        retry_strategy_config=RetryStrategyConfig(max_attempts=3),
...
def test_integration_with_create_retry_strategy():
    ...
    # Use a real RetryStrategyConfig with specific settings
    config = WithRetryConfig(
        retry_strategy_config=RetryStrategyConfig(
            max_attempts=4,
            ...
            jitter_strategy=__import__(
                "aws_durable_execution_sdk_python.config", fromlist=["JitterStrategy"]
            ).JitterStrategy.NONE,
        ),
