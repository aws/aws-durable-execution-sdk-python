...
class WaitOperationPluginFactory:
    """Builds one :class:`WaitOperationPlugin` for each invocation.
    ...
    def create_plugin(self, info: InvocationStartInfo) -> WaitOperationPlugin:
        """Return this invocation's plugin. ``info`` is unused."""
        return WaitOperationPlugin()


@durable_execution(plugins=[WaitOperationPluginFactory()])
